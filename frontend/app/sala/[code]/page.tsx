...
export default function SalaPage() {
  ...
  useEffect(() => {
    ...
    async function initRoom() {
      try {
        ...
        alert('Sala não encontrada ou indisponível.');
        router.push('/');
      }
    }
  ...
  function shareInviteOnWhatsApp() {
    const link = `${window.location.origin}/sala/${code}`;
    const text = `Jogue Stop comigo no Jogos da Galera!\n\nSala ${code}: ${link}`;
      ...
      <section className="mx-auto grid max-w-5xl gap-6">
        <div className="flex flex-col gap-4 rounded-2xl bg-white p-6 shadow-sm md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-zinc-600">Compartilhe o código ou link com seus amigos.</p>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <Card>
            ...
            <div className="mt-4 flex gap-2">
              <Input placeholder="Digite uma mensagem" value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') sendMessage(); }} />
