...
export function StopGame({ code, playerId }: StopGameProps) {
              ...
              <div className="mt-5 grid gap-4">
                {round.categories.map((category) => (
                  <div key={category}>
                    ...
                    <Input disabled={blocked} placeholder={`${category} com ${round.letter}`} value={answers[category] || ''} onChange={(event) => updateAnswer(category, event.target.value)} />
                  </div>
                ))}
              </div>
